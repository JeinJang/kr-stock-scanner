...
## 구현 중 확인한 결과 (2026-08-20 실측)

11년 백필을 실제로 완료하고 세 항목을 모두 측정했다. 전체 수치: **5,740 요청 / 5,392 거래일 적재 / 6,481,499행 / 12.1분 / 742MB / 수정 이벤트 3,365건.**

- **임계값 0.02의 아래쪽** — 실제 이벤트가 있었다. 648만 행에서 정상 거래일은 계수 편차가 **정확히 0**이고, 0.5~2% 구간에 **253건**이 실재했다(56%가 12월 말 주식배당 권리락). 0.1~0.5%(371건)와 0~0.1%(22건)는 같은 티커가 인접일에 동일 편차를 반복하는 반올림 잡음이었다. **임계값을 0.005로 낮췄고** 검출 이벤트가 3,112 → 3,365건이 됐다.
- **검출 이벤트 상위 50건 육안 확인** — 42건이 정수배(10배 38, 50배 2, 25배 2)로 액면분할. 삼성전자 보통주·우선주가 같은 날 동일하게 50배로 잡혀 교차 검증됐다. 비정수배 8건 중 2건을 표본 확인해 오리온 인적분할(20.36배)과 대규모 감자(76.14배)로 설명됐다. 설명되지 않는 건은 없었으나 **비정수배 6건은 미확인**이다.
- **실제 용량** — **742MB**. 스펙이 상한으로 잡은 780MB 이내다. 27일 표본으로 외삽했던 832MB는 과대추정이었다(작은 DB일수록 인덱스 오버헤드 비중이 크다).
